...
// Evita que um valor inválido na coluna ImagemURL (ex.: um número que
// vazou de outra coluna por engano) vire um <img src="90"> no navegador,
// que o browser tenta buscar como se fosse um caminho relativo do site
// (https://seusite.netlify.app/90) e gera um 404.
function sanitizeImagePath(raw) {
  const value = String(raw ?? '').trim();
  if (!value) return '';
  // Só aceita valores que parecem caminho/URL de imagem: têm "/" ou "."
  // e não são puramente numéricos.
  const looksNumeric = /^\d+$/.test(value);
  const looksLikePath = /[./]/.test(value);
  if (looksNumeric || !looksLikePath) return '';
  return value;
}
  ...
  async getCharacters() {
    ...
    const data = rows.map((r, i) => {
      ...
      return {
        ...
        rarity,
        image: sanitizeImagePath(r.get('ImagemURL')),
        costs,
      };
    }).filter(c => c.name);
