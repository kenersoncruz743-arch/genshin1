...
const USER_CHARS_HEADER = ['UserId', 'Personagem', 'Constelacao', 'NivelPersonagem', 'ArmaNome', 'ArmaRefinamento', 'ArmaNivel', 'AtualizadoEm'];
...
class SheetsService {
  ...
  async getCharacters() {
    ...
    const enkaByName = {};
    if (enkaMaps) {
      for (const c of Object.values(enkaMaps.charMap)) enkaByName[c.name.toLowerCase()] = c;
    }
    ...
    const data = rows.map((r, i) => {
      ...
      const enkaMatch = enkaByName[name.toLowerCase()];
      const sheetImage = String(r.get('ImagemURL') || '').trim();
      return {
        id: 'c' + i,
        name,
        element: String(r.get('Elemento') || '').trim(),
        rarity,
        image: (enkaMatch && enkaMatch.image) || sheetImage,
        costs,
      };
    }).filter(c => c.name);
  ...
  async getWeapons() {
    ...
    const enkaByName = {};
    if (enkaMaps) {
      for (const w of Object.values(enkaMaps.weaponMap)) enkaByName[w.name.toLowerCase()] = w;
    }
  ...
  async upsertUserCharacter(userId, characterName, constellation, build) {
    ...
    const fields = {
      ...
      NivelPersonagem: build && build.characterLevel ? build.characterLevel : '',
      ArmaNome: build && build.weapon ? build.weapon.name : '',
      ArmaRefinamento: build && build.weapon ? build.weapon.refinement : '',
      ArmaNivel: build && build.weapon ? build.weapon.level : '',
  ...
  async importFromUID(userId, uid) {
    ...
    for (const av of profile.characters) {
      ...
      used += cost;
      importados.push({ name: catalogMatch.name, constellation, cost, temDetalhes: av.temDetalhes });
    }
