...
class SheetsService {
  ...
  async importFromUID(userId, uid) {
    ...
    for (const av of profile.characters) {
      const catalogMatch = (await this.getCharacters()).find(c => c.name === av.name);
      if (!catalogMatch) {
        ignorados.push({ name: av.name, motivo: 'Personagem não encontrado na aba Personagens da planilha.' });
        continue;
      }
      ...
      if (limit !== null && used + cost > limit) {
        ignorados.push({ name: av.name, motivo: `Ultrapassaria o limite de pontos do deck (${limit}).` });
        continue;
      }

      await this.upsertUserCharacter(userId, av.name, constellation);
      used += cost;
      importados.push({ name: av.name, constellation, cost, temDetalhes: av.temDetalhes });
    }
