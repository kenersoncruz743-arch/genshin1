...
const USER_CHARS_HEADER = ['UserId', 'Personagem', 'Constelacao', 'NivelPersonagem', 'ArmaNome', 'ArmaRefinamento', 'ArmaNivel', 'ArtefatosJSON', 'AtualizadoEm'];
...
class SheetsService {
  ...
  async getOrCreateSheet(title, header) {
    ...
    if (!sheet) {
      ...
      // recurso) mas a planilha já existia sem ela, cria a coluna em vez de
      // quebrar silenciosamente ao tentar ler/gravar nela.
  ...
  async listUserCharacters(userId) {
    ...
    return rows
      ...
      .map(r => {
        let artifacts = [];
        const raw = r.get('ArtefatosJSON');
        if (raw) {
          try { artifacts = JSON.parse(raw); } catch { artifacts = []; }
        }
        return {
