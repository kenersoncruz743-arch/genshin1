...
const USER_CHARS_HEADER = ['UserId', 'Personagem', 'Constelacao', 'Arma', 'ArmaRefinamento', 'Build', 'BuildDetalhes', 'AtualizadoEm'];
...
class SheetsService {
  ...
  async getOrCreateSheet(title, header) {
    ...
    if (!sheet) {
      ...
      // A function fica "quente" entre chamadas na Netlify — sem isso, se a
      // estrutura de colunas da planilha mudar (ex: reimportar um xlsx novo),
      // o mapeamento nome-de-coluna → índice ficaria desatualizado até a
      // function reiniciar, lendo valores da coluna errada.
      await sheet.loadHeaderRow();
      // Se o código passou a esperar colunas novas (ex: Arma/Build) que essa
      // aba ainda não tem, adiciona elas no fim sem apagar nada que já
      // existia — assim não é preciso editar a planilha na mão.
      const missing = header.filter(h => !sheet.headerValues.includes(h));
      if (missing.length) {
  ...
  async listUserCharacters(userId) {
    ...
    return rows
      ...
      .map(r => {
        let buildDetalhes = null;
        const raw = r.get('BuildDetalhes');
        if (raw) {
          try { buildDetalhes = JSON.parse(raw); } catch { buildDetalhes = null; }
        }
        return {
          character_name: r.get('Personagem'),
          constellation: Number(r.get('Constelacao') || 0),
          weapon_name: r.get('Arma') || '',
          weapon_refinement: r.get('ArmaRefinamento') ? Number(r.get('ArmaRefinamento')) : null,
          build: r.get('Build') || '',
          build_detalhes: buildDetalhes, // { weapon, artifacts, stats, level, icon } — ver lib/enka.js
        };
      });
  }
  ...
  // extras (opcional): { weaponName, weaponRefinement, build, buildDetalhes }.
  // Quando não informado (ex: o jogador só mudou a constelação num
  // personagem que já tinha build salva), o que já estava gravado é
  // preservado. buildDetalhes é o objeto { weapon, artifacts, stats, icon }
  // vindo de lib/enka.js, guardado como JSON — usado pra abrir o card com
  // os artefatos completos ao clicar no personagem.
  async upsertUserCharacter(userId, characterName, constellation, extras) {
    ...
    // Célula do Google Sheets aguenta até ~50.000 caracteres — o JSON da
    // build fica na casa de 2-4 mil, mas por segurança corta em vez de
    // quebrar a gravação caso venha algo anormal.
    let buildDetalhesStr;
    if (extras && extras.buildDetalhes !== undefined) {
      const json = extras.buildDetalhes ? JSON.stringify(extras.buildDetalhes) : '';
      buildDetalhesStr = json.length > 45000 ? '' : json;
    }
    ...
    if (existing) {
      ...
      if (extras && extras.weaponName !== undefined) existing.set('Arma', extras.weaponName || '');
      if (extras && extras.weaponRefinement !== undefined) existing.set('ArmaRefinamento', extras.weaponRefinement || '');
      if (extras && extras.build !== undefined) existing.set('Build', extras.build || '');
      if (buildDetalhesStr !== undefined) existing.set('BuildDetalhes', buildDetalhesStr);
      ...
      await sheet.addRow({
        ...
        Arma: (extras && extras.weaponName) || '',
        ArmaRefinamento: (extras && extras.weaponRefinement) || '',
        Build: (extras && extras.build) || '',
        BuildDetalhes: buildDetalhesStr || '',
  ...
  async importFromUID(userId, uid) {
    ...
    for (const av of profile.characters) {
      ...
      if (!catalogMatch) {
        ignorados.push({ name: av.name, motivo: `"${av.name}" não está cadastrado na aba Personagens da planilha.` });
        continue;
      }
      ...
      await this.upsertUserCharacter(userId, catalogMatch.name, constellation, {
        weaponName: av.weapon ? av.weapon.name : '',
        weaponRefinement: av.weapon ? av.weapon.refinement : '',
        build: av.build || '',
        buildDetalhes: av.temDetalhes ? {
          icon: av.icon || null,
          level: av.level || null,
      ...
      used += cost;
      importados.push({ name: catalogMatch.name, constellation, cost, temDetalhes: av.temDetalhes, weapon: av.weapon, build: av.build });
    }
